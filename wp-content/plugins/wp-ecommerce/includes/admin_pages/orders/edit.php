...
if( $order_id ){
    $objWp2023Order = new WpOrder();
    $order = $objWp2023Order->find($order_id);
    $order_items = $objWp2023Order->order_items($order_id);

}
if (isset($_POST['wp_save_order'])){

    check_admin_referer('wp-update_order');
    ...
    $objWp2023Order = new WpOrder();
    $order = $objWp2023Order->update($order_id, [
        'status' => $order_status,
        'note' => $note,
...
<div class="wrap">
    ...
    <form id="posts-filter" method="post">
        <?php wp_nonce_field('wp-update_order') ;?>
        <div id="poststuff">
                            ...
                            <div class="submitbox">
                                <p>
                                    <select name="order_status" class="large-text ">
                                        <option <?= $order->status == 'pending' ? 'selected' : ''; ?> selected=""
                                            value="pending">Đơn
                                            hàng mới</option>
                                        <option <?= $order->status == 'completed' ? 'selected' : ''; ?>
                                            value="completed">Đơn đã hoàn
                                            thành</option>
                                        <option <?= $order->status == 'canceled' ? 'selected' : ''; ?> value="canceled">
                                            Đơn đã hủy
                                        </option>
                                    </select>
                                </p>
                                <p>
                                    <input type="submit" name="wp_save_order" id="submit" class="button button-primary"
                                        value="Lưu thay đổi">
                                </p>
                            </div>
                        </div>
